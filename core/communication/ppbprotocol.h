...
// Состояния машины
enum class PPBState {
    Idle,               // Ничего не делаем, нет подключения
    Ready,              // Связь есть, можно отправлять команды
    SendingCommand,     // Отправили команду, ждем OK
    WaitingData         // ждём данные (статус, тестовые пакеты)
};
...
#pragma pack(push, 1)
struct DataPacket {
    uint8_t data[2];      // 3 байта данных
    uint8_t counter;      // 1 байт номера пакета, от 0
 ...
 * WARNING: изменена структура пакеты. третий байт в пакете ДАННЫХ это его номер(с нуля) для правильного позиционирования.
 * при отправке данных мы тоже третим байтом указываем номер(при командах VOLUME и PRB)
 * полученные пакеты сохраняются в оперативку. Сравниваются с отправленными. Логируются так |отправленные|полученные|. После получения запускаем анализатор.
 *  Если все в логи пишется данные сошлись. Если не сходится пишется не сошлось\строки
 * удаляем данные.
 *
 * Если признак ФУ:
 ...
 * подключение больше не сбрасываем.
